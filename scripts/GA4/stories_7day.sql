IF (EXISTS( 
    SELECT 1 FROM `daring-phoenix-295916.analytics_248462478.events_*` 
    ...
    AND (NOT EXISTS( 
        ...
        WHERE table_name = 'ga4_stories_7_day' 
        AND date(creation_time) = CURRENT_DATE('America/Chicago') )
        ) 
    THEN CREATE OR REPLACE TABLE `custom_reporting.ga4_stories_7_day` AS 
    ...
    WITH pageviews AS( 
        ...
        -- (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'article_enforcement_level') as article_enforcement_level, 
            COUNT(*) as pageviews, COUNT(DISTINCT concat(user_pseudo_id, " ", (SELECT value.int_value FROM unnest(event_params) WHERE key = "ga_session_id") )) as sessions, 
            COUNT(DISTINCT user_pseudo_id ) as users 
            FROM `daring-phoenix-295916.analytics_248462478.events_*` 
            WHERE _TABLE_SUFFIX BETWEEN FORMAT_DATE("%Y%m%d", DATE_SUB(CURRENT_DATE('America/Chicago'), INTERVAL 7 DAY)) AND FORMAT_DATE("%Y%m%d", DATE_SUB(CURRENT_DATE('America/Chicago'), INTERVAL 1 DAY)) 
    ...
    meta_data AS ( 
        SELECT (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'page_name') as page_name, 
    ...
    most as( 
        SELECT t.article_id, 
        t.full_url, t.author, t.publish_date, t.publish_dow, t.time_published, t.page_section, t.all_sections,
        t.subsection, t.page_level, t.content_source, p.pageviews, p.sessions, p.users,t.time_published_24hr 
        FROM ( 
            SELECT *, 
            ROW_NUMBER() OVER (PARTITION BY article_id ORDER BY pageviews DESC) rn 
            FROM meta_data ) t 
            LEFT JOIN pageviews p ON t.article_id = p.article_id WHERE t.rn = 1) 
                
        SELECT 
            m.*, 
            md.page_name 
            FROM ( SELECT page_name,article_id, ROW_NUMBER() OVER (PARTITION BY article_id ORDER BY pageviews DESC) rn FROM meta_data ) md 
            LEFT JOIN most m ON m.article_id = md.article_id 
            WHERE md.rn=1 ORDER BY pageviews DESC; 
END IF;